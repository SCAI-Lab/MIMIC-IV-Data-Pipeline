...
import model.model_utils as model_utils
importlib.reload(model.parameters)
import model.parameters as parameters
from model.parameters import *
...
#                     print(Y_train.shape)
                    ...
                    # Saving each variable to an HDF5 file
                    meds.to_hdf(self.data_dt_folder + 'meds.h5', key='meds', mode='w', complevel=9, complib='blosc')
                    chart.to_hdf(self.data_dt_folder + 'chart.h5', key='chart', mode='w', complevel=9, complib='blosc')
                    out.to_hdf(self.data_dt_folder + 'out.h5', key='out', mode='w', complevel=9, complib='blosc')
                    proc.to_hdf(self.data_dt_folder + 'proc.h5', key='proc', mode='w', complevel=9, complib='blosc')
                    lab.to_hdf(self.data_dt_folder + 'lab.h5', key='lab', mode='w', complevel=9, complib='blosc')
                    stat_train.to_hdf(self.data_dt_folder + 'stat_train.h5', key='stat_train', mode='w', complevel=9, complib='blosc')
                    demo_train.to_hdf(self.data_dt_folder + 'demo_train.h5', key='demo_train', mode='w', complevel=9, complib='blosc')
                    Y_train.to_hdf(self.data_dt_folder + 'Y_train.h5', key='Y_train', mode='w', complevel=9, complib='blosc')

                    save_time = time.time() - start_time
                    print(f"Time taken to save all variables: {save_time} seconds")


                    start_time = time.time()

                    # Loading each variable from its corresponding HDF5 file
                    meds = pd.read_hdf(self.data_dt_folder + 'meds.h5', key='meds')
                    chart = pd.read_hdf(self.data_dt_folder + 'chart.h5', key='chart')
                    out = pd.read_hdf(self.data_dt_folder + 'out.h5', key='out')
                    proc = pd.read_hdf(self.data_dt_folder + 'proc.h5', key='proc')
                    lab = pd.read_hdf(self.data_dt_folder + 'lab.h5', key='lab')
                    stat_train = pd.read_hdf(self.data_dt_folder + 'stat_train.h5', key='stat_train')
                    demo_train = pd.read_hdf(self.data_dt_folder + 'demo_train.h5', key='demo_train')
                    Y_train = pd.read_hdf(self.data_dt_folder + 'Y_train.h5', key='Y_train')

                    load_time = time.time() - start_time
                    print(f"Time taken to load all variables: {load_time} seconds")
                    """
...
#         print(logits.shape)
    ...
    def create_model(self,model_type):
        if model_type=='Time-series LSTM':
            ...
            self.net = LSTMBaseH(self.device,
                               self.cond_vocab_size,
                               self.proc_vocab_size,
                               self.med_vocab_size,
                               self.out_vocab_size,
